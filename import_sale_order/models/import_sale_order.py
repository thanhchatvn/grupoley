...
class ImportChartAccount(models.TransientModel):
    ...
    def valid_product_code(self, archive_lines, product_obj):
        ...
        for line in archive_lines:
            ...
            product_id = product_obj.search([('x_product_supplierinfo.product_code', '=', code)])
            ...
            if not product_id:
                error = ("Linea de pedido: {}\n\n"
                         "No existe ningun producto con el código [{}] en el sistema.").format(counter,code)
                raise UserError(error)
    ...
    def valid_customer(self, archive_lines, customer_obj):
        ...
        for line in archive_lines:
            ...
            if len(partner_id) > 1:
                error = ("Linea de pedido: {}\n\n"
                         "El cliente esta duplicado en el sistema.").format(counter)
                raise UserError(error)
